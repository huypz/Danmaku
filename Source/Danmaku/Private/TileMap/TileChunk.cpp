...
void ATileChunk::TriangulateTile()
{
	int32 Index = 0;
	for (const UTileCell* Cell : Cells)
	{
		InstancedStaticMeshComponent->SetTranslucentSortPriority(static_cast<int32>(Cell->TileType));
...
void ATileChunk::TriangulateBlend()
{
	int32 Index = Cells.Num();
